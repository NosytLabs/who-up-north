...
- **Map geometry:** Statistics Canada 2021 province/territory digital boundary layer (GeoJSON).
...
The core model therefore advances locally without a server. The Pages workflow is scheduled every 15 minutes to refresh population, release-wire, official map, and public-signal snapshots; GitHub may delay scheduled jobs. The browser separately attempts direct read-only refreshes of the weather, Open Government and Bank of Canada cards. The Time Use Survey itself is a survey profile, not a live sensor feed.
